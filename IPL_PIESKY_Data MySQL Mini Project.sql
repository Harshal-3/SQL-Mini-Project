...
SELECT * FROM IPL_BIDDING_DETAILS;
SELECT BIDDER_ID,SUM(CASE WHEN BID_STATUS='WON' THEN 1 ELSE 0 END)AS WIN ,
SUM(CASE WHEN BID_STATUS='WON' THEN 1 ELSE 0 END )*100/COUNT(*) AS WIN_PER 
FROM IPL_BIDDING_DETAILS GROUP BY BIDDER_ID ORDER BY WIN_PER DESC;
...
SELECT S.STADIUM_NAME,S.CITY,COUNT(M.SCHEDULE_ID) 'NO.OF MATCHES'
...
SELECT MS.STADIUM_ID,COUNT(*) 'NO.OF MATCHES' ,
SUM(CASE WHEN M.TOSS_WINNER=1 AND M.MATCH_WINNER =1 THEN 1 ELSE 0 END) TOTAL_TOSS_GAME_WINNER,
((SUM(CASE WHEN M.TOSS_WINNER=1 AND M.MATCH_WINNER =1 THEN 1 ELSE 0 END))/COUNT(*))*100 AS WIN_PER
...
SELECT M.WIN_DETAILS,T.TEAM_ID
FROM IPL_MATCH M
JOIN IPL_TEAM T
ON M.TEAM_ID1=T.TEAM_ID
WHERE SUBSTR(M.WIN_DETAILS,6,3)=T.REMARKS;
...
SELECT TS.TEAM_ID,T.TEAM_NAME,SUM(TS.MATCHES_PLAYED) AS MATCHES_PLAYED , 
SUM(TS.MATCHES_WON) AS TOTAL_WON ,
SUM(TS.MATCHES_LOST) AS TOTAL_LOST
...
SELECT P.PLAYER_ID,P.PLAYER_NAME,TP.PLAYER_ROLE
FROM IPL_PLAYER P
...
SELECT T.TEAM_ID,T.TEAM_NAME,TP.PLAYER_ROLE,COUNT(TP.PLAYER_ROLE) AS 'NO.OF ALL ROUNDER'
FROM IPL_TEAM T 
JOIN IPL_TEAM_PLAYERS TP
